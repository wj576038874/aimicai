...
public class Banner2Adapter extends DelegateAdapter.Adapter<RecyclerView.ViewHolder> {
    ...
    public Banner2Adapter(Context context, List<String> images, RecyclerView.RecycledViewPool viewPool, LinearLayoutHelper linearLayoutHelper) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == 1) {
            return new BannerHolder(LayoutInflater.from(context).inflate(R.layout.banner2, parent, false));
        } else {
...
//        }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder.itemView instanceof XBanner) {
            ...
            xBanner.setData(images, null);//第二个参数为提示文字资源集合
    ...
    class BannerPagerAdapter extends RecyclablePagerAdapter<RecyclerView.ViewHolder> {
        ...
        BannerPagerAdapter(Banner2Adapter adapter, RecyclerView.RecycledViewPool pool) {
            super(adapter, pool);
        }
